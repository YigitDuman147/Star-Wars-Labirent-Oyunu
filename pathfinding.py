...
def bfs_double_step(maze, start_location, target_location):
    ...
    while queue:
        ...
        if (current_loc.get_x() == target_location.get_x() and
                current_loc.get_y() == target_location.get_y()):
            print(f"Kylo Ren Yol Buldu: {[(loc.get_x(), loc.get_y()) for loc in path]}")
            return path
        double_neighbors = maze.get_double_step_neighbors(current_loc)
        print(f"Kylo Çift Adım Komşuları: {[(n.get_x(), n.get_y()) for n in double_neighbors]}")
        ...
        print(f"Kylo Normal Adım Komşuları: {[(n.get_x(), n.get_y()) for n in normal_neighbors]}")
